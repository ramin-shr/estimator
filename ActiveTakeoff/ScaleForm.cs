...
namespace QuoterPlan
{
    public partial class ScaleForm : BaseForm
    {
        ...
        {
            bool needsDefaultScale = unitScale.Scale == 0f && !unitScale.MustSetManually;
        ...
        {
            bool isImperialOrEngineering = (this.cbScale.SelectedIndex == 0 || this.cbScale.SelectedIndex == 2);
            bool isEngineering = (this.cbScale.SelectedIndex == 2);

            this.lblScale_1.Visible = isEngineering;

            if (!this.unitScale.MustSetManually && !this.setManually)
            {
                this.cbImperialScale.Visible = (isImperialOrEngineering && !isEngineering);
                this.lblScaleReference.Text = ((isImperialOrEngineering && !isEngineering) ? "1' 0\" =" : ((!isEngineering) ? "1 :" : "1\" ="));
                this.txtScale.Visible = (!isImperialOrEngineering || isEngineering);

                if (!isImperialOrEngineering || isEngineering)
                {
                    this.txtScale.Text = ((this.txtScale.Text == "") ? "0" : this.txtScale.Text);
                }

                if (isImperialOrEngineering && !isEngineering)
                    Utilities.SetObjectFocus(this.cbImperialScale);
                else
                    Utilities.SetObjectFocus(this.txtScale);
            ...
            {
                this.lblManual_1.Visible = isImperialOrEngineering;
                this.txtManualFeet.Visible = isImperialOrEngineering;
                this.lblManual_2.Visible = true;
                this.txtManualInches.Visible = isImperialOrEngineering;
                this.txtManualMetric.Visible = !isImperialOrEngineering;
                this.lblManual_2.Text = (isImperialOrEngineering ? "\"" : "mm");
            }

            this.lblScaleReference.Text = (isImperialOrEngineering ? "1\" =" : "10 mm =");
            this.ComputeManualScale();

            if (this.unitScale.MustSetManually)
            {
                if (isImperialOrEngineering)
                    Utilities.SetObjectFocus(this.txtManualFeet);
                else
                    Utilities.SetObjectFocus(this.txtManualMetric);
            }
            else
            {
                Utilities.SetObjectFocus(this.btModify);
            }
        }
        ...
        private void txtNumeric_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (allowed.IndexOf(e.KeyChar) == -1 && e.KeyChar != '\r' && e.KeyChar != '\b')
            {
                e.Handled = true;
            }
        }
        ...
        private float ComputeManualScale()
        {
            ...
            {
                scaleFactor = this.unitScale.ScaleFactor;
            }
            else
            {
                ...
                {
                    int totalInches = Utilities.ConvertToInt(this.txtManualFeet.Text) * 12 + Utilities.ConvertToInt(this.txtManualInches.Text);
                    if (totalInches > 0)
                    {
                        scaleFactor = (float)this.unitScale.ReferenceDistance / (float)totalInches;
                    }
                }
                else
                {
                    int metricUnits = Utilities.ConvertToInt(this.txtManualMetric.Text) / 10;
                    if (metricUnits > 0)
                    {
                        scaleFactor = (float)this.unitScale.ReferenceDistance / (float)metricUnits;
                    }
                }
            }

            this.txtScale.Text = Math.Round((double)scaleFactor, 3).ToString() + " pixels";
            return scaleFactor;
        }
        ...
        private void btOk_Click(object sender, EventArgs e)
        {
            ...
            if (!this.unitScale.MustSetManually)
            {
                if (unitSystem != UnitScale.UnitSystem.undefined)
                {
                    if (unitSystem == UnitScale.UnitSystem.imperial)
                    {
                        if (!isEngineering)
                        {
                            isValid = (this.cbImperialScale.SelectedItem != null);
                            if (isValid)
                            {
                                scaleValue = (float)((Utilities.ItemData)this.cbImperialScale.SelectedItem).Data;
                            }
                        }
                        else
                        {
                            scaleValue = (float)Utilities.ConvertToInt(this.txtScale.Text);
                            isValid = (scaleValue > 0f);
                        }
                    }
                    else
                    {
            ...
            {
                scaleValue = this.ComputeManualScale();
                isValid = (scaleValue > 0f);
            }

            if (isValid)
            {
                this.unitScale.SetScale(scaleValue, unitSystem, this.unitScale.Precision, this.unitScale.MustSetManually);
                this.unitScale.Engineering = isEngineering;
            ...
            if (!this.unitScale.MustSetManually)
            {
                if (unitSystem == UnitScale.UnitSystem.imperial && !isEngineering)
                    Utilities.SetObjectFocus(this.cbImperialScale);
                else
                    Utilities.SetObjectFocus(this.txtScale);
        ...
        {
            bool needsDefaultScale = (this.unitScale.Scale == 0f && !this.unitScale.MustSetManually);

            if (needsDefaultScale)
            {
                if (this.unitScale.ScaleSystemType == UnitScale.UnitSystem.imperial)
                    Utilities.SetObjectFocus(this.cbImperialScale);
                else
                    Utilities.SetObjectFocus(this.txtScale);
